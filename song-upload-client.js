...
// and saves the song; a progress bar reaching 100% does not mean published.
export async function transferSong(file,{kind,request,requestId,onProgress=()=>{},isCurrent=()=>true}) {
  const limit=100*1024*1024;
  if(!file?.size || file.size>limit) throw new Error('Choose a song up to 100 MB.');
  const active=()=>{if(!isCurrent())throw new DOMException('Upload canceled','AbortError');};
  active();
  const start=await request('/api/audio-uploads/start',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({request_id:requestId,kind,name:file.name,size:file.size})});
  ...
  for(let part=0;part<start.parts;part++) {
    ...
    for(let attempt=0;attempt<3;attempt++) {
      active();
      try {result=await request(`/api/audio-uploads/part?upload_id=${encodeURIComponent(start.upload_id)}&part=${part}`,{method:'POST',headers:{'Content-Type':'application/octet-stream'},body:bytes});break;}
