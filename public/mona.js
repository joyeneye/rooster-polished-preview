...
export function createMonaWorkspace({document: doc = document, window: win = window, fetch: requestFetch = (...args) => window.fetch(...args)} = {}) {
  ...
  let epoch = 0, workspace = null, profile = {}, bookings = null, results = [], busy = false, conflicted = false, dead = false;
  ...
  function clearPrivate() {
    ...
    workspace = null; profile = {}; bookings = null; results = []; conflicted = false;
  ...
  function preferences() {
    return {profession:byId('mona-profession').value.trim(),location:byId('mona-location').value.trim(),offering:byId('mona-offering').value.trim(),goal:byId('mona-goal').value};
  }
  function fillPreferences(saved) {
    const described = win.RoosterProfileProfession?.describe(profile) || {};
    const defaults = {profession:described.label || profile.profession || '',location:profile.location || '',offering:'',goal:'clients'};
    for (const key of ['profession','location','offering','goal']) {
  ...
  async function boot() {
    ...
    try {
      ...
      }).catch(error => { if (!error.stale && generation === epoch) profile = {}; }),loadBookings(generation)];
      await Promise.allSettled(pending);
    } catch (error) { if (!error.stale && generation === epoch) showGate('Mona couldn’t open your work yet.',error.message,{retry:true}); }
  }
  listen(form,'input',() => { form.dataset.edited = 'true'; if (results.length) { results = []; renderResults(); byId('mona-search-status').textContent = 'Your work details changed. Find opportunities again for fresh matches.'; } });
  listen(form,'submit',async event => {
    ...
    try {
      const data = await request('/api/mona/scout',{method:'POST',body:JSON.stringify(values)});
