...
// A whole song must never be sent through one small Functions request.
export const AUDIO_UPLOAD_LIMIT = 100 * 1024 * 1024;
export const AUDIO_PART_BYTES = 2 * 1024 * 1024;
...
type Manifest = {id: string; member_id: string; kind: 'profile' | 'discovery'; size: number; name: string; mime: string; parts: number; expires_at: number};
export function audioTransferStore(context: Context): TransferStore {
  const options = {name: 'member-audio-transfers', consistency: 'strong' as const};
...
export async function boundedBytes(req: Request, limit: number): Promise<Uint8Array> {
  if (!req.body) throw new MemberError(400, 'The upload was empty. Choose your song again.');
...
export async function smallAudioJSON(req: Request): Promise<any> {
  if ((req.headers.get('content-type') || '').split(';')[0].trim().toLowerCase() !== 'application/json') throw new MemberError(415, 'Refresh your music editor and try again.');
...
async function manifest(store: TransferStore, member: string, id: string): Promise<Manifest> {
  if (!MEMBER_ID.test(id)) throw new MemberError(400, 'Invalid song upload.');
  const value = await store.get(`${base(member, id)}/manifest`, {type:'json'});
  if (!value || value.id !== id || value.member_id !== member || !['profile','discovery'].includes(value.kind) ||
      !Number.isSafeInteger(value.size) || value.size < 1 || value.size > AUDIO_UPLOAD_LIMIT ||
      value.parts !== Math.ceil(value.size / AUDIO_PART_BYTES) || typeof value.name !== 'string' || typeof value.mime !== 'string') throw new MemberError(404, 'This song upload was not found. Choose the file again.');
  if (!Number.isFinite(value.expires_at) || value.expires_at <= Date.now()) throw new MemberError(410, 'This upload expired. Choose your song again.');
  return value;
}
function failure(error: unknown) { return error instanceof MemberError ? memberJSON({error:error.message}, error.status) : memberJSON({error:'Your song upload could not connect. Please try again.'},503); }
export async function startAudioUpload(req: Request, store: TransferStore, resolve: MemberResolver = requireCommunityMember): Promise<Response> {
  if (req.method !== 'POST') return memberJSON({error:'Method not allowed.'},405);
  try {
    assertSameOrigin(req); const member = await resolve(); const input = await smallAudioJSON(req);
    if (!MEMBER_ID.test(input.request_id || '') || !['profile','discovery'].includes(input.kind)) throw new MemberError(400,'Refresh your music editor and try again.');
    if (!Number.isSafeInteger(input.size) || input.size <= 0 || input.size > AUDIO_UPLOAD_LIMIT) throw new MemberError(413,'Choose a song up to 100 MB.');
    const name = typeof input.name === 'string' ? input.name.slice(0,120) : '';
    const ext = name.split('.').at(-1)?.toLowerCase();
    if (!(input.kind === 'profile' ? ['mp3'] : ['mp3','m4a','wav']).includes(ext || '')) throw new MemberError(415, input.kind === 'profile' ? 'Profile music uses MP3. Export an MP3 and try again.' : 'Choose an MP3, M4A or WAV.');
    const id = input.request_id.toLowerCase(), key = `${base(member.id,id)}/manifest`;
    const candidate: Manifest = {id,member_id:member.id,kind:input.kind,size:input.size,name,mime:ext==='mp3'?'audio/mpeg':ext==='wav'?'audio/wav':'audio/mp4',parts:Math.ceil(input.size/AUDIO_PART_BYTES),expires_at:Date.now()+TTL};
    ...
    if (saved.size !== candidate.size || saved.name !== candidate.name || saved.kind !== candidate.kind) throw new MemberError(409,'This upload belongs to a different file. Choose your song again.');
    return memberJSON({upload_id:id,part_bytes:AUDIO_PART_BYTES,parts:saved.parts,expires_at:saved.expires_at});
  } catch(error) { return failure(error); }
}
export async function putAudioPart(req: Request, store: TransferStore, resolve: MemberResolver = requireCommunityMember): Promise<Response> {
  if (req.method !== 'POST') return memberJSON({error:'Method not allowed.'},405);
  try {
    ...
    const expected = Math.min(AUDIO_PART_BYTES,saved.size-part*AUDIO_PART_BYTES);
    const bytes = await boundedBytes(req,AUDIO_PART_BYTES);
    ...
    if(!written.modified) { const existing=await store.get(key,{type:'arrayBuffer'});
      if(!existing || digest(existing)!==digest(bytes.buffer as ArrayBuffer)) throw new MemberError(409,'This upload part changed. Choose your song again.'); }
    return memberJSON({upload_id:id,part,received:bytes.byteLength});
  } catch(error) { return failure(error); }
}
export async function readAudioUpload(store: TransferStore | undefined, member: string, id: string, kind: 'profile'|'discovery'): Promise<File> {
  if(!store) throw new MemberError(503,'Large song uploads are not available yet. Refresh after the site update.');
  const saved=await manifest(store,member,String(id||'').toLowerCase());
  if(saved.kind!==kind) throw new MemberError(400,'This song was selected for a different upload form.');
  ...
  for(let part=0;part<saved.parts;part++) {
    ...
    const expected=Math.min(AUDIO_PART_BYTES,saved.size-part*AUDIO_PART_BYTES);
    if(!(bytes instanceof ArrayBuffer) || bytes.byteLength!==expected) throw new MemberError(409,'Your song has not finished uploading. Tap the upload button to resume.');
    data.set(new Uint8Array(bytes),part*AUDIO_PART_BYTES);
  }
  // Keep the parts until expiry so an interrupted final save can be retried.
  return new File([data],saved.name,{type:saved.mime});
}
export async function cleanAudioTransfers(store: TransferStore): Promise<number> {
